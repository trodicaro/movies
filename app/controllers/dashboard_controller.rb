class DashboardController < ApplicationController
  def index
    ...
    @daily_sales= @orders.order("date_trunc('day', screenings.scheduled_at) ASC, orders.total_cost ASC").pluck("date_trunc('day', screenings.scheduled_at)", :total_cost).group_by(&:first).map {|day, records| [day.strftime("%a, %m-%d"),records.map(&:last).reduce(:+)]}.transpose
  end
end
